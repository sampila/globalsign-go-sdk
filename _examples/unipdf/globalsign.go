...
import (
	...
	"globalsign-go-sdk/examples/unipdf/sign_handler"
)
...
func (s *globalsignDssSigner) Load() error {
	...
	m, err := globalsign.NewManager(&globalsign.ManagerOption{
		...
		CertificatePath: s.certFilepath,
	})
	if err != nil {
		return err
	}
...
func (s *globalsignDssSigner) Sign(ctx context.Context, rd *model.PdfReader, option *SignOption) (*pdf.PdfAppender, error) {
	...
	handler, err := sign_handler.NewGlobalSignDSS(context.Background(), s.manager, option.SignedBy, signerIdentity)
	if err != nil {
		return nil, err
	}
