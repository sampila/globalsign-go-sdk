...
import (
	...
	"github.com/sampila/globalsign-go-sdk"
...
func (h *GlobalsignDSS) Sign(sig *model.PdfSignature, digest model.Hasher) error {
	...
	cb := func(rand io.Reader, digest []byte, opts crypto.SignerOpts) ([]byte, error) {
		// sign digest
		signature, err := h.manager.Sign(h.ctx, h.signer, &globalsign.IdentityRequest{SubjectDn: globalsign.SubjectDn{}}, digest)
		if err != nil {
	...
	err = signedData.RequestSignerTimestampToken(0, func(digest []byte) ([]byte, error) {
		...
		t, err := h.manager.Timestamp(h.ctx, h.signer, &globalsign.IdentityRequest{SubjectDn: globalsign.SubjectDn{}}, hasher.Sum(nil))
		if err != nil {
